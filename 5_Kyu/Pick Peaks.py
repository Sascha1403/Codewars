...
# Funktion
def pick_peaks(arr):
    ### definiere pos und peak
    pos = []
    peaks = []

    index = 1

    ### Iteriere über alle Elemente von i=2 bis i<n
    while index < len(arr)-1:

        ### if Rechts und links kleiner füge index und peak hinzu
        if arr[index - 1] < arr[index] and arr[index + 1] < arr[index]:
            pos.append(index)
            peaks.append(arr[index])


        ### if Rechts gleich und links kleiner
        if arr[index - 1] < arr[index] and arr[index + 1] == arr[index]:

            ### gehe solange nach rechts bis element kleiner oder größer ist aber nicht mehr als n
